...
/*		Looper.myQueue().addIdleHandler(new MessageQueue.IdleHandler()
	...
	public void run()
	{
		...
		try
		{
			...
			RunUnNetHack(mState.getDataDir());
		}
		catch(Exception e)
		{
			Log.print("EXCEPTED");
	...
	{
		final String msg = new String(cmsg);
		if(wid == mMessageWid)
			Log.print(msg);
	...
	{
		final String question = new String(cquestion);
	...
	private String getLine(final byte[] title, final int nMaxChars, final int showLog, int reentry)
	{
		if(reentry == 0)
		{
			final String msg = new String(title);
	...
	{
		final String msg = new String(text);
		mHandler.post(new Runnable()
		{
			public void run()
			{
	...
	{
		final String msg = new String(prompt);
